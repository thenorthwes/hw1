...
def write_new_training_data(training_data_path, unk_map):
    # input file
    fin = open(training_data_path, "rt")
    # output file to write the result to
    fout = open(TRAINING_UNKED_DATA_, "wt")
    # for each line in the input file
    for line in fin:
        tokens = line.split(" ")
        unkedLine = ""
        for token in tokens:
            unkedLine += unk_map.get(token, token) + " "
...
class ngram:
    def __init__(self, training_data_path, ngram_size):
        ...
        self.calculate_unkspace(training_data_path)
        path_to_unked_data = write_new_training_data(training_data_path, self.unk_map)
    ...
    def calculate_unkspace(self, training_data_path):
        training_data = open(training_data_path, "r")
        sentence = training_data.readline()
        # loop through first and unk low occurrence words
        while sentence:
            ...
            for vocab_instance in sentence_tokens:
                # test for if this is a valid ngram with this size
                self.vocabulary_space[vocab_instance] = self.vocabulary_space.get(vocab_instance, 0) + 1 # count every word
                self.total_ngrams += 1  # count up the total number of ngrams we see
            sentence = training_data.readline()
        training_data.close()
        # make our unks
        unked_word_counter = 0
        for vocab_count in self.vocabulary_space.keys():
            if self.vocabulary_space[vocab_count] <= UNK_THRESHOLD and unked_word_counter < MAX_UNKS:
                self.unk_map[vocab_count] = UNK_
                unked_word_counter += 1
